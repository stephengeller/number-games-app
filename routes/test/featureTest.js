...
describe("App", function() {
	...
	describe("/", function() {
		it("works", function(done) {
			browser.visit(url, function() {
				expect(browser.text("body")).to.include(
					"Dr. PJs MakersBnB Creepy houses for all your occasions!"
				);
				browser.assert.element(
					'form button[type="submit"][name="Visit Rooms"]'
				);
		...
		it("shows rooms when you click Visit Rooms", function(done) {
			browser.visit(url, function() {
				...
				browser.click("#viewrooms", function() {
		...
		it("shows sign in page when you click Sign In", function(done) {
			browser.visit(url, function() {
				browser.click("#signin", function() {
					browser.assert.element('form input[type="text"][name="username"]');
					browser.assert.element(
						'form input[type="password"][name="password"]'
					);
					browser.assert.element('form button[name="sign in"]');
					done();
				});
			});
		});
	...
	describe("users", function() {
		...
		it("goes to the room page after signing up", function(done) {
			browser.visit(url + "signup", function() {
				browser
					.fill("input[name=name]", "Kay Lovelace")
					.fill("input[name=username]", "kaylove")
